...
function Tip({ active, payload }) {
  ...
  return (
    <div className="card px-3 py-2 text-xs fig shadow-lg">
      <div className="font-medium text-ink mb-1">{p.label || "Allocation"}</div>
...
function Dot({ label, color, faded }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-muted">
      <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: color, opacity: faded ? 0.3 : 1 }} />
  ...
  return (
    <div className="card card-hover p-5">
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-2xs text-subtle">{frontier.portfolios.length.toLocaleString()} simulated allocations</span>
      </div>
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 8, right: 16, left: 0, bottom: 4 }}>
            ...
            />
            <ZAxis range={[16, 16]} />
            <Tooltip content={<Tip />} cursor={{ stroke: "#CBD5E1" }} />
            <Scatter data={cloud} fill="#6366F1" fillOpacity={0.13} isAnimationActive={false} />
            <Scatter
              data={[make(frontier.min_vol_metrics, "Min volatility")]}
              isAnimationActive={false}
              shape={(p) => <circle cx={p.cx} cy={p.cy} r={6} fill="#0D9488" stroke="#fff" strokeWidth={1.5} />}
            />
            <Scatter
              data={[make(frontier.optimal_metrics, "Max Sharpe")]}
              isAnimationActive={false}
              shape={(p) => <circle cx={p.cx} cy={p.cy} r={6} fill="#059669" stroke="#fff" strokeWidth={1.5} />}
            />
            <Scatter
              data={[make(currentMetrics, "Your portfolio")]}
              isAnimationActive={false}
              shape={(p) => (
                <g>
                  <circle cx={p.cx} cy={p.cy} r={11} fill="#6366F1" fillOpacity={0.18} />
                  <circle cx={p.cx} cy={p.cy} r={7} fill="#6366F1" stroke="#fff" strokeWidth={2.5} />
                </g>
              )}
            />
          </ScatterChart>
      ...
      <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border-base">
